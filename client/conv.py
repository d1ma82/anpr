...
BASE_PATH="./data/chars/"
project_name="ANPR"
train_csv_file=BASE_PATH+"train/train.tfrecords"
test_csv_file=BASE_PATH+"test/test.tfrecords"
image_resize=[20,20]

@tf.function
def model_fn(features, labels, mode, params):

    convolutional_2d_1537261701724 = tf.keras.layers.Conv2D(
...
def data_train_estimator():
    ...
    iterator = iter(dataset) #dataset.make_one_shot_iterator()  # create one shot iterator
    feature, label = next(iterator) #iterator.get_next()
...
def run_experiment(args):
    ...
    train_spec = tf.estimator.TrainSpec(input_fn=data_train_estimator, max_steps=200)
